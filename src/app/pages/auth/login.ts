...
@Component({
  selector: 'app-login',
  standalone: true,
  imports: [ButtonModule, CheckboxModule, InputTextModule, PasswordModule, FormsModule, RouterModule, RippleModule, AppFloatingConfigurator, Toast, NgIf, Dialog, EscolaFormComponent],
  template: `
        ...
        <div
          ...
          <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
            <div class="text-center mb-8">
              ...
              <div class="text-surface-900 dark:text-surface-0 text-3xl font-medium mb-4">Bem Vindo ao Prova Facíl
              </div>
            ...
            <form #loginForm="ngForm" (ngSubmit)="login()">
              ...
              <input pInputText id="email1" type="text" placeholder="Email" #emailField="ngModel"
                     class="w-full md:w-[30rem] mb-8" [(ngModel)]="email" name="email" required/>

              <span *ngIf="emailField.invalid && emailField.touched" class="text-danger flex -mt-4 text-red-700 mb-4">
              ...
              <div class="flex items-center justify-between mt-2 mb-8 gap-8">
                <div class="flex items-center">
                  <button type="button"
                          class="font-medium no-underline ml-2 text-right cursor-pointer text-primary bg-transparent border-0"
                          (click)="tipoConta = true">
                    Criar Conta
                  </button>
                </div>
                <button type="button"
                        class="font-medium no-underline ml-2 text-right cursor-pointer text-primary bg-transparent border-0"
                        (click)="isRecuperarSenha = true">
                  Esqueceu a senha?
                </button>
              </div>
    ...
    <div>
      <p-dialog header="Selecione o tipo de conta que deseja criar" [modal]="true" [(visible)]="tipoConta">
        <div class="flex justify-center gap-2">
          <p-button label="Professor" icon="bi bi-person" routerLink="/auth/criar-conta" class="p-button-text"
                    variant="outlined" severity="info"/>
          <p-button label="Escola" icon="bi bi-building" routerLink="/auth/criar-escola" class="ml-4 p-button-text"
                    variant="outlined"
                    severity="success"/>
        </div>
      </p-dialog>

      <p-dialog header="Recuperação de senha" [modal]="true" [(visible)]="isRecuperarSenha">
        <label class="block text-surface-900 dark:text-surface-0 text-xl font-medium mb-2">Email</label>
        <input pInputText id="email1" type="text" placeholder="Email" #emailField="ngModel"
               class="w-full md:w-[30rem] mb-8" [(ngModel)]="emailRecuperacao" name="emailRecuperacao" required/>

        <div class="flex justify-center gap-2">
          <p-button label="Confirmar" class="p-button-text" variant="outlined" severity="success"
                    (onClick)="recuperarSenha()"/>
          <p-button label="Cancelar" class="ml-4 p-button-text" variant="outlined"
                    severity="danger" (click)="isRecuperarSenha = false"/>
        </div>
      </p-dialog>
    </div>
  `
