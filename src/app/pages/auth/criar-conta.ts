...
@Component({
    selector: 'app-access',
    standalone: true,
  imports: [ButtonModule, RouterModule, RippleModule, AppFloatingConfigurator, ButtonModule, InputText, Password, ReactiveFormsModule, FormsModule,  Toast],
      ...
      <div class="flex flex-col items-center justify-center">
        <div style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
          <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
            ...
            <div>
              <label for="email1" class="block text-surface-900 dark:text-surface-0 text-xl font-medium mb-2">Nome</label>
              <input pInputText id="nome" type="text" placeholder="Nome Completo" class="w-full md:w-[30rem] mb-8" [(ngModel)]="nome" />

              <label for="email1" class="block text-surface-900 dark:text-surface-0 text-xl font-medium mb-2">Email</label>
              <input pInputText id="email1" type="email" placeholder="Email" class="w-full md:w-[30rem] mb-8" [(ngModel)]="email" />

              <label for="password1" class="block text-surface-900 dark:text-surface-0 font-medium text-xl mb-2">Senha</label>
              <p-password id="password1" [(ngModel)]="senha" placeholder="Senha" [toggleMask]="true" styleClass="mb-4" [fluid]="true" [feedback]="false"></p-password>

              <p-button label="Finalizar" [outlined]="true"  styleClass="w-full mt-5" (onClick)="cadastrar()"></p-button>
              <p-button label="Cancelar" [outlined]="true" severity="danger"  styleClass="w-full mt-5" (onClick)="Cancelar()"></p-button>
            </div>
          </div>
        </div>
      </div>
